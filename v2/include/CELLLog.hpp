...
public:
	...
	void setLogPath(const char* logPath, const char* mode)
	{
		if (_logFile)
		{
			Info("CELLLog::fclose : _logFile is not empty...\n");
			fclose(_logFile);
			_logFile = nullptr;
		}
	...
	static void Info(const char* pStr)
	{
		...
		pLog->_taskServer.addTask([=]() {
			if (pLog->_logFile)
			{
				...
				fprintf(pLog->_logFile, "[%d-%d-%d %d:%d:%d]", now->tm_year + 1900, now->tm_mon + 1, now->tm_mday, (now->tm_hour + 8) % 24, now->tm_min, now->tm_sec);
